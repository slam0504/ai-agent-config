...
| 檔案 | 狀況 | 處理 |
|---|---|---|
| `claude/CLAUDE.md` | 本機 `~/.claude/CLAUDE.md` 於 2026-08-26 被直接編輯，「Subagent 委派」一節擴寫成「Subagent 委派與模型分工（Fable 主腦 + Sonnet 執行）」，repo 來源落後 | 正式來源維持原版；本機版本放在本目錄 `CLAUDE.md` 待審，差異可用 `diff claude/CLAUDE.md claude/review/2026-09-25/CLAUDE.md` 檢視。核准前這台機器的 `./install.sh` 會被 drift guard 擋下該檔，屬預期行為 |
...
| `permissions.allow` | 39 筆 | 保留 34 筆；4 筆含 `/Users/<user>/` 絕對路徑者移至本機範例；1 筆含公司內部 Go module 主機名稱的一次性 `find` 權限直接捨棄，不寫入 public repo |
...
需要在審核時有意識決定的權限相關旗標（候選原樣保留）：`permissions.defaultMode = "auto"`、`skipDangerousModePermissionPrompt`、`skipAutoPermissionPrompt`、`useAutoModeDuringPlan`。
...
18 個測試在修正版全部通過；同一組測試複製到 `~/.claude/hooks/` 原版上執行，16 個失敗，其餘為正向對照組。
...
- hooks 回歸測試 18 個通過；所有 `.py` 通過 `py_compile`、所有 `.sh` 通過 `bash -n`。
...
4. ~~hooks 修正核可後，同步回本機~~ 已完成（2026-09-25）。
